...
def run(num_app_rsu1,num_app_rsu2,num_task,fn_bs,fn_app,delay_constraints,cache_capacity):
    ...
    for i in range(800):
        ...
        while not done:
            ...
            state = new_state
            score += reward
            if deep_dqn.memory_counter > MEMORY_CAPACITY:
                deep_dqn.learn()
        ...
        rate_record.append((env.local_success+env.offload_success+env.offload_huancun)/(env.local_success+env.offload_success+env.offload_huancun+env.local_unsuccess+env.offload_unsuccess))
    ...
    df.to_excel("excel_files/episode_SR/6.xlsx")
